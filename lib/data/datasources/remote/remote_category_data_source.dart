...
class RemoteCategoryDataSource implements CategoryDataSource {
  ...
  Future<List<Category>> getCategories() async {
    DebugLogger.log('RemoteCategoryDataSource: getCategories() started',
    ...
    try {
      ...
      DebugLogger.log('Exception in getCategories: $e',
          category: 'DATA_SOURCE');
  ...
  Future<List<SubCategoryMenu>> getSubCategoriesMenu(int categoryId) async {
    ...
    if (response.statusCode == 200) {
      try {
        ...
        if (!responseJson.containsKey('subcategories') ||
            responseJson['subcategories'] == null) {
        ...
        if (subcategoriesData is List) {
          ...
          throw Exception(
              '⚠️ خطأ: استجابة غير صحيحة من الخادم، البيانات ليست قائمة.');
        }
      } catch (e) {
