...
import 'package:talabna/data/models/report.dart';
...
class UserFollowRepository {
  ...
  Future<List<User>> getUserSeller({int page = 1}) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('auth_token');
  ...
  Future<Reports> makeReport(
      {required int id, required String type, required String reason}) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('auth_token');

    final response = await http.post(
        Uri.parse(
            '$_baseUrl/api/reports/reported/$type/reportedId/$id/reason/$reason'),
        headers: {'Authorization': 'Bearer $token'});
  ...
  Future<Map<String, dynamic>> searchUserOrPost({
    required String searchAction,
    int page = 1,
  }) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('auth_token');
    ...
    if (response.statusCode == 200) {
      ...
      final List<User> userResult =
          (data["users"]["data"] as List).map((e) => User.fromJson(e)).toList();
