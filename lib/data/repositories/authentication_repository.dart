...
class AuthenticationRepository {
  ...
  static const String _webClientId =
      '808302489355-m69df866rpeoacp0p29ro61c1kpra2da.apps.googleusercontent.com'; // Add your Web Client ID from Firebase
  ...
  Future<GoogleSignInAuthentication> _getGoogleAuth(GoogleSignInAccount account,
      {int retries = 2}) async {
    for (int i = 0; i <= retries; i++) {
      try {
        DebugLogger.log(
            'Requesting Google authentication tokens (attempt ${i + 1})',
            category: 'AUTH');
        final auth = await account.authentication;

        // Verify we actually have the tokens
        if (auth.idToken == null) {
          DebugLogger.log('ID token is null on attempt ${i + 1}',
              category: 'AUTH_ERROR');
          throw Exception('Failed to obtain Google ID token');
        }

        if (auth.accessToken == null) {
          DebugLogger.log('Access token is null on attempt ${i + 1}',
              category: 'AUTH_ERROR');
        ...
        DebugLogger.log('Error getting authentication (attempt ${i + 1}): $e',
            category: 'AUTH_ERROR');
  ...
  Future<Map<String, dynamic>> _post(String url, dynamic data,
      {String? token}) async {
    try {
      ...
      DebugLogger.log('Response status code: ${response.statusCode}',
          category: 'HTTP');
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        try {
          ...
          String errorMessage =
              errorData['error'] ?? 'Status code error: ${response.statusCode}';
          DebugLogger.log('API error: $errorMessage', category: 'HTTP_ERROR');
          throw Exception(errorMessage);
        } catch (e) {
          DebugLogger.log('Cannot parse error response: $e',
              category: 'HTTP_ERROR');
  ...
  Future<Map<String, dynamic>> _get(String url,
    ...
    try {
      ...
      final response =
          await http.get(Uri.parse('$API_BASE_URL/$url'), headers: headers);
  ...
  Future<Map<String, dynamic>> login({
    ...
    return {'userId': userId, 'token': authToken, 'authType': authType};
  }
  ...
  Future<Map<String, dynamic>> register({
    ...
    DebugLogger.log('Registration successful for user ID: $userId',
        category: 'AUTH');
    ...
    try {
      ...
      if (authType == 'google') {
        try {
          ...
          DebugLogger.log('Error signing out from Google: $e',
              category: 'AUTH_ERROR');
        }
      }
  ...
  Future<bool> checkTokenValidity(String token) async {
    try {
      DebugLogger.logAuth(
          action: 'VALIDATING', source: 'AuthRepository', token: token);
  ...
  Future<void> saveAuthToken(String token) async {
    ...
    if (success) {
      DebugLogger.logToken(
          token: token, action: 'SAVED', source: 'SharedPreferences');
    } else {
  ...
  Future<void> saveUserId(int userId) async {
    ...
    if (success) {
      DebugLogger.log('UserId saved: $userId', category: 'AUTH');
    } else {
  ...
  Future<void> saveAuthType(String authType) async {
    ...
    if (success) {
      DebugLogger.log('Auth type saved: $authType', category: 'AUTH');
    } else {
  ...
  Future<String?> getAuthType() async {
    ...
    return prefs.getString(_authTypeKey) ??
        'email'; // Default to email if not set
  }

  Future<void> resetPassword({required String email}) async {
    try {
      DebugLogger.log('Requesting password reset for email: $email',
          category: 'AUTH');
      ...
      if (response.statusCode != 200) {
        ...
        DebugLogger.log(
            'Password reset request failed: ${errorData['error'] ?? 'Unknown error'}',
            category: 'AUTH_ERROR');
        throw Exception(
            errorData['error'] ?? 'Failed to send password reset link');
      }

      DebugLogger.log('Password reset link sent successfully',
          category: 'AUTH');
    } catch (error) {
      DebugLogger.log('Error sending password reset link: $error',
          category: 'AUTH_ERROR');
  ...
  Future<Map<String, dynamic>> signInWithGoogle() async {
    try {
      ...
      if (isSignedIn) {
        DebugLogger.log('User already signed in with Google, signing out first',
            category: 'AUTH');
        await _googleSignIn.signOut();
      }
      ...
      if (googleSignInAccount == null) {
        DebugLogger.log('Google sign-in was cancelled by user',
            category: 'AUTH');
        throw Exception('Google sign in cancelled by user');
      }

      DebugLogger.log('Google account selected: ${googleSignInAccount.email}',
          category: 'AUTH');

      // Get authentication with retry mechanism
      try {
        final GoogleSignInAuthentication googleAuth =
            await _getGoogleAuth(googleSignInAccount);

        // Check if we have tokens
        final String? idToken = googleAuth.idToken;
        final String? accessToken = googleAuth.accessToken;

        DebugLogger.log('ID token received: ${idToken != null}',
            category: 'AUTH');
        DebugLogger.log('Access token received: ${accessToken != null}',
            category: 'AUTH');

        if (idToken == null) {
          DebugLogger.log('Failed to obtain Google ID token',
              category: 'AUTH_ERROR');
          throw Exception('Failed to obtain Google ID token');
        }

        if (accessToken == null) {
          DebugLogger.log('Failed to obtain Google access token',
              category: 'AUTH_ERROR');
          throw Exception('Failed to obtain Google access token');
        }
        ...
        DebugLogger.log('Received response from server for Google auth',
            category: 'AUTH');
        ...
        if (authToken == null) {
          DebugLogger.log('Server did not return an auth token',
              category: 'AUTH_ERROR');
          throw Exception('Authentication token not found in response');
        }

        if (userId == null) {
          DebugLogger.log('Server did not return a user ID',
              category: 'AUTH_ERROR');
        ...
        if (responseData['user'] != null &&
            responseData['user']['data_saver_enabled'] != null) {
          final bool dataSaverEnabled =
              responseData['user']['data_saver_enabled'] == true;
          final prefs = await SharedPreferences.getInstance();
          await prefs.setBool(_dataSaverKey, dataSaverEnabled);
        }

        DebugLogger.log('Google sign-in successful for user ID: $userId',
            category: 'AUTH');
        ...
        DebugLogger.log('Error during Google authentication: $e',
            category: 'AUTH_ERROR');
        ...
        try {
          ...
          DebugLogger.log(
              'Error signing out after authentication failure: $signOutError',
              category: 'AUTH_ERROR');
        }
      ...
      DebugLogger.log('Error signing in with Google: $error',
          category: 'AUTH_ERROR');
  ...
  Future<bool> setDataSaverStatus(bool isEnabled) async {
    ...
    if (authToken != null) {
      try {
        ...
        DebugLogger.log('Error updating data saver on server: $e',
            category: 'AUTH_ERROR');
