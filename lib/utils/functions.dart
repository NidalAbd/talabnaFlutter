...
void checkBadgeAndShowMessage(
    BuildContext context, String selectedHaveBadge, int selectedBadgeDuration) {
  // Check if the selected badge requires a minimum balance
  final int haveBadge = selectedHaveBadge == 'ذهبي'
      ? 2
      : selectedHaveBadge == 'ماسي'
          ? 10
          : 0;
  ...
  if (userProfileState is UserPointLoadSuccess) {
    ...
    if (balance < minBalance) {
      final message =
          'لا يوجد رصيد كافي لتمييز النشر بـ $selectedHaveBadge لمدة $selectedBadgeDuration يوم';
