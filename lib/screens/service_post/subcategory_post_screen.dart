...
class SubCategoryPostScreenState extends State<SubCategoryPostScreen> {
  ...
  void _loadInitialData() {
    // Add debug log
    print(
        'Loading initial data for subcategory ${widget.subcategoryId} with reported posts count');
        ...
        body: BlocConsumer<ServicePostBloc, ServicePostState>(
          ...
          listener: (context, state) {
            if (state is ServicePostLoadSuccess) {
              print(
                  'Received ${state.servicePosts.length} posts for subcategory ${widget.subcategoryId}');
          ...
          builder: (context, state) {
            ...
            if (state is ServicePostLoadFailure) {
              print('Failed to load posts: ${state.errorMessage}');
