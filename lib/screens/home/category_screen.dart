...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  List<CategoryMenu> arrangeCategories(List<CategoryMenu> categories) {
    try {
      ...
      final otherCategories =
          categories.where((category) => category.id != 7).toList();
  ...
  Future<bool> _loadShowSubcategoryGridView() async {
    try {
      ...
      DebugLogger.log('Error loading grid view preference: $e',
  ...
  Future<void> _saveShowSubcategoryGridView(bool value) async {
    try {
      ...
      DebugLogger.log('Error saving grid view preference: $e',
          category: 'PREFERENCES');
    }
  }
          ...
          if (selectedCategory == null && categories.isNotEmpty) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                setState(() {
                  try {
                    selectedCategory = categories.firstWhere((c) => c.id == 1,
                        orElse: () => categories.first);
                  } catch (e) {
                    // Handle edge case if list is empty for some reason
                    DebugLogger.log('Error auto-selecting category: $e',
                        category: 'CATEGORIES');
                  }
                });
              }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        _subcategoryBloc
                            .add(FetchCategories(forceRefresh: true));
                      },
  ...
  Widget _buildCategoryCard(CategoryMenu category) {
    ...
    try {
      ...
      DebugLogger.log('Error getting category name: $e',
          category: 'CATEGORIES');
    }
    ...
    return Padding(
      ...
      child: RawChip(
        onPressed: () {
          ...
          DebugLogger.log('Selected category: ${category.id} ($displayName)',
              category: 'CATEGORIES');
        },
