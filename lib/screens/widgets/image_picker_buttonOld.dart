...
class ImagePickerButtonState extends State<ImagePickerButton> {
      ...
      if (_pickedImages.length + pickedFiles.length > maxImages) {
        ...
        for (XFile file in pickedFiles) {
          ...
          final img.Image? compressedImage =
              await _compressImage(File(filePath));
          if (compressedImage != null) {
            ...
            setState(() {
              ...
              _processingImages.add(jpegPath!);
            });

            await _submitLocalImages();
    ...
    if (pickedFile != null) {
      ...
      if (_pickedImages.length + 1 > maxImages) {
        ...
        if (videoFile.lengthSync() <= maxSizeBytes &&
          ...
          setState(() {
            _pickedImages
                .add(Photo.fromJson({'src': pickedFile.path, 'isVideo': true}));
          ...
          if (mp4Path != null) {
            setState(() {
              _pickedImages
                  .add(Photo.fromJson({'src': mp4Path, 'isVideo': true}));
    ...
    if (originalImage != null) {
      ...
      if (originalSize > maxSize) {
        ...
        await _saveImageToFile(
            compressedImageData, '${file.path}_compressed.jpg');
  ...
  Future<String?> _convertToMP4(String videoPath) async {
    ...
    final String mp4Path =
        '$tempPath/${DateTime.now().millisecondsSinceEpoch}.mp4';
  ...
  Future<void> _submitLocalImages() async {
    ...
    for (String? imagePath in _processingImages) {
      setState(() {
  ...
  void _removeImage(int index) async {
    ...
    if (widget.deleteApi) {
      context
          ...
          .add(DeleteServicePostImageEvent(servicePostImageId: photoUrl!));
    }
              ...
              if (index == _pickedImages.length) {
                ...
                return Stack(
                  children: [
                    ...
                    Image(
                      image: (_localImages.isNotEmpty &&
                              _localImages[index] != null)
                          ? (_pickedImages[index].isVideo ?? false)
                              ? ThumbnailImage(videoPath: _localImages[index]!)
                                  as ImageProvider
