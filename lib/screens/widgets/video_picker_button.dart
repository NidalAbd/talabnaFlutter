...
class VideoPickerButtonState extends State<VideoPickerButton> {
  ...
  Future<void> _pickVideos() async {
    ...
    if (videoFile != null) {
      ...
      final String? compressedVideoPath =
          await _compressAndConvertToMP4(videoPath);
      if (compressedVideoPath != null) {
        setState(() {
          final Photo pickedVideo =
              Photo.fromJson({'src': compressedVideoPath});
  ...
  Future<String?> _compressAndConvertToMP4(String videoPath) async {
    ...
    final Duration trimmedDuration =
        videoDuration < limitDuration ? videoDuration : limitDuration;
  ...
  void _removeLocalImage(int index) {
    setState(() {
      ...
      if (widget.deleteApi) {
        final List<Photo> apiImages = getApiImages();
        final int apiImageIndex = _pickedImages
            .indexOf(apiImages.firstWhere((photo) => photo.src == localImage));
                ...
                children: [
                  Image.network(
                    ...
                    errorBuilder: (BuildContext context, Object exception,
                      ...
                      return Image.asset(
                        'assets/talabnaLogo512.png',
                        // Replace with the path to your default thumbnail image
                        fit: BoxFit.cover,
                        width: double.infinity,
                      );
                    },
