...
class _ThemeToggleListTileState extends State<ThemeToggleListTile> {
  ...
  void _showCircularTransitionAnimation(
      BuildContext context, Offset localPosition) {
...
class CircularTransitionOverlay extends StatefulWidget {
  ...
  const CircularTransitionOverlay(
      {super.key, required this.tapPosition, required this.color});

  @override
  _CircularTransitionOverlayState createState() =>
      _CircularTransitionOverlayState();
}
...
class _CircularTransitionOverlayState extends State<CircularTransitionOverlay>
  ...
  double _calculateRadius(Size size) {
    final double x =
        max(widget.tapPosition.dx, size.width - widget.tapPosition.dx);
    final double y =
        max(widget.tapPosition.dy, size.height - widget.tapPosition.dy);
    return sqrt(x * x + y * y);
  }
}
...
class CircularTransitionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = color.withOpacity(0.5); // Change the opacity here
