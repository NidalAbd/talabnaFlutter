...
class UserContact extends StatelessWidget {
    ...
    if (status.isGranted) {
      try {
        final newContact = Contact(
          ...
          emails: [if (email != null && email!.isNotEmpty) Email(email!)],
        );
        ...
        if (context.mounted) {
          showCustomSnackBar(context, _language.contactAddedSuccess(username),
              type: SnackBarType.success);
        }
      } catch (e) {
        if (context.mounted) {
          showCustomSnackBar(context, _language.contactAddError(),
      ...
      // Permission denied
      if (context.mounted) {
        showCustomSnackBar(context, _language.permissionDeniedMessage(),
            type: SnackBarType.error);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // Reduced padding for tight grouping
      child: ListTile(
        leading: Container(
