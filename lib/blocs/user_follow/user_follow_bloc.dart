...
import 'package:talabna/blocs/user_follow/user_follow_event.dart';
import 'package:talabna/blocs/user_follow/user_follow_state.dart';
...
class UserFollowBloc extends Bloc<UserFollowEvent, UserFollowState> {
  ...
  UserFollowBloc({required UserFollowRepository repository})
    ...
    on<UserFollowerRequested>((event, emit) async {
      ...
      try {
        final users = await _repository.getFollowerByUserId(
            userId: event.user, page: event.page);
        bool hasReachedMax = users.length <
            10; // Assuming 10 is the maximum number of items you fetch in one request
        emit(UserFollowerFollowingSuccess(
            users: users, hasReachedMax: hasReachedMax));
      } catch (e) {
    ...
    on<UserFollowingRequested>((event, emit) async {
      emit(UserFollowLoadInProgress());
      try {
        final users = await _repository.getFollowingByUserId(
            userId: event.user, page: event.page);
        bool hasReachedMax = users.length <
            10; // Assuming 10 is the maximum number of items you fetch in one request
        emit(UserFollowerFollowingSuccess(
    ...
    on<UserSellerRequested>((event, emit) async {
      ...
      try {
        ...
        bool hasReachedMax = users.length <
            10; // Assuming 10 is the maximum number of items you fetch in one request
        emit(
            UserSellerSuccessState(users: users, hasReachedMax: hasReachedMax));
      } catch (e) {
        emit(UserFollowLoadFailure(error: e.toString()));
      }
    });
