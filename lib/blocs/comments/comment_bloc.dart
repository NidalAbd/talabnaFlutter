...
import 'package:talabna/data/repositories/comment_repository.dart'; // Import your Comment repository
...
class CommentBloc extends Bloc<CommentEvent, CommentState> {
  ...
  Future<void> _onLoadMoreComments(
      LoadMoreCommentsEvent event, Emitter<CommentState> emit) async {
    if (isFetching) return;
    isFetching = true;
    if (state is CommentLoadSuccessState &&
        (state as CommentLoadSuccessState).hasMore) {
      try {
        final newComments = await commentRepository.fetchComments(
            postId: event.postId, page: event.page);
        final currentState = state as CommentLoadSuccessState;
        final allComments = currentState.comments + newComments;
        final hasMore = newComments
            .isNotEmpty; // or determine based on the API response if there are more pages
  ...
  Future<void> _onAddComment(
    ...
    try {
      ...
      final comments = await commentRepository.fetchComments(
        postId: event.comment.servicePostId,
        page: event.page,
      );
      emit(CommentLoadSuccessState(comments));
    } catch (e) {
