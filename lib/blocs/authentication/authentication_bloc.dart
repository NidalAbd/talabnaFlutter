...
import 'package:talabna/utils/debug_logger.dart'; // Ensure this import exists
...
class AuthenticationBloc
  ...
  AuthenticationBloc({
    ...
    const _debounceTime = Duration(seconds: 2);
    // Login Event Handler
    ...
    on<GoogleSignInRequest>((event, emit) async {
      ...
      if (_lastGoogleSignInAttempt != null &&
          now.difference(_lastGoogleSignInAttempt!).compareTo(_debounceTime) <
              0) {
        DebugLogger.log('Ignoring rapid Google Sign-In request',
            category: 'AUTH');
        return;
      }
      ...
      if (_isProcessingGoogleSignIn) {
        DebugLogger.log(
            'Google Sign-In already in progress, ignoring duplicate request',
            category: 'AUTH');
        return;
      }
      ...
      try {
        ...
        // Important: Use a delay to prevent immediate duplicate attempts
    ...
    on<LoggedIn>((event, emit) async {
      ...
      try {
        ...
        DebugLogger.log('Authentication check error: $e',
            category: 'AUTH_ERROR');
    ...
    on<SetDataSaverEnabled>((event, emit) async {
      if (state is AuthenticationSuccess) {
        ...
        try {
          ...
          DebugLogger.log('Error setting data saver: $e',
              category: 'AUTH_ERROR');
        }
      }
    });
